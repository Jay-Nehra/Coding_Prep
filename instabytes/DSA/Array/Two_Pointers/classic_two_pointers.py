...
def pair_sum(numbers: List[int], target: int) -> List[Tuple[int, int]]:
    ...
    while left < right:
        logger.info(
            f"Numbers at the left pointer:  {numbers[left]}, and number at the right pointer: {numbers[right]}"
        )
    ...
    logger.info(
        f"These are the total pair from the array which are equal to the target sum:    {result}"
    )
    return result
...
def user_input() -> Tuple[List[int], int]:
    while True:
        ...
        target = input(
            "Please enter the target that we are finding the pair sum for:   "
        )
        try:
...
def main():
    ...
    logger.info(
        f"Here are the pairs from the array that sum upto the target:   {pairs}"
    )
