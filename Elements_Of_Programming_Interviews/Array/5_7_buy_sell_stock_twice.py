...
def maximum_profit(prices: List[int]) -> int:
    ...
    for i, price in enumerate(prices):
        ...
        logger.debug(
            f"Day {i}: price={price}, minimum_price_so_far={minimum_price_so_far}, "
    ...
    for i in reversed(range(1, len(prices))):
        ...
        logger.debug(
            f"Day {i}: price={prices[i]}, maximum_price_so_far={maximum_price_so_far}, "
...
def user_input() -> List[int]:
    while True:
        ui = input(
            "Please Enter the Stock Price History. It should be space seperated interger values. \n"
        )
        try:
...
def main():
    ...
    logger.info(
        f"Maximum profit for selling and buyin the stock twice is:   {max_profit}"
    )
