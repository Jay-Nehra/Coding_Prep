...
def increment(decimal_encoded_array: List[int]) -> List[int]:
    ...
    for i in reversed(range(1, len(decimal_encoded_array))):
        ...
        if decimal_encoded_array[i] != 10:
            break
        decimal_encoded_array[i], decimal_encoded_array[i-1] = 0, decimal_encoded_array[i-1]+1
...
if __name__ == '__main__':
    DEA = input("Please enter the space seperated integer array that represent the decimal encoded number: \n\n")
    DEA = DEA.split()
