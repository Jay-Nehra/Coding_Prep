...
def replace_and_remove(characters: List[str]) -> Tuple[List[str], int]:
    ...
    for i, char in enumerate(characters):
        logger.info(
            f"Current char in the array is: {char} and the input array index is: {i}."
        )
        if char != "b":
    ...
    logger.info(
        f"Array after forward pass: {characters[:write_index]} with a_count = {a_count}"
    )
    ...
    for j in reversed(range(write_index)):
        logger.info(
            f"Current char in the array is: {characters[j]} and the input array index is: {j}."
        )
        char = characters[j]
        if char == "a":
...
def user_input() -> List[str]:
    while True:
        chars = input(
            "Please enter a space-separated list of characters (e.g., a c b a b): \n"
        )
        try:
            chars = chars.split()
            ...
            logger.error(
                f"Could not parse the input. Please enter valid single characters. Error: {e}"
            )
