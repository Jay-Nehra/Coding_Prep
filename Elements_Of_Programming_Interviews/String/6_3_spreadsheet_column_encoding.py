...
def user_input() -> str:
    while True:
        ...
        if column_id.isalpha() and column_id.isupper():
            ...
            logger.error("Invalid input. Please enter a valid column encoding using uppercase letters only.")
...
def main():
    ...
    logger.info(f"The integer encoding for the column '{column_id}' using the optimal approach is: {result_optimal}")
    logger.info(f"The integer encoding for the column '{column_id}' using the brute-force approach is: {result_bf}")
